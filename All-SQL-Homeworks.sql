...
where ad.date_of_birth < '01-04-1992'
...
select * from artist_details ad
where lower(ad.full_name) like '%e%'
...
select s.name as song_name, sl.lyrics as song_lyrics from song s
join song_lyrics sl
on s.id = sl.song_id;
...
select a.name as artist_name, ad.country as country from artist a 
join artist_details ad
on a.id = ad.artist_id;
...
select artist.name as artist_name, album.name as album_name, album.rating 
from album
...
select artist.name as artist_name, song.name as song_name, song.explicit
from song
join artist on song.id = artist.id
...
select a.name as artist_name, count(s.id) as song_count, max(s.duration) as max_duration, avg(s.duration) as avg_duration from artist a
join song s on a.id = s.artist_id
group by a.name
...
create view vw_ArtistSongCount as
...
from vw_ArtistSongCount
order by song_count

-- Create a new view (vw_ArtistAlbumDetails) that will list all artists (name) and count the albums they have
create view vw_ArtistAlbumDetils as
select a.name as artist_name, count(al.id) as album_count
from artist a
join song s on a.id = s.artist_id
join album al on s.album_id = al.id
group by a.name 
...
-- 1. Calculate rating of album by artist
create view vw_ArtistAlbumRating as
select a.name as artist_name, avg(al.rating) as album_count
from artist a
join song s on a.id = s.artist_id
join album al on s.album_id = al.id
group by a.id, a.name

-- 2. Get longest song by genre
select g.name as genre, s.name as song_name, s.duration
from song s
inner join songs_genres sg on s.id = sg.song_id
inner join genre g on sg.genre_id = g.id
group by g.name, s.name, s.duration
having s.duration = (
  select MAX(duration)
  from song s2
  inner join songs_genres sg2 on s2.id = sg2.song_id
  inner join genre g2 on sg2.genre_id = g2.id
  where g2.name = g.name
);

-- 3. Create text like "<Artist_Name> best rated album is <Name_of_album>”
create view vw_ArtistBestRatedAlbum as
with BestRatedAlbums as (
    select a.id as artist.id, a.name as artist_name, al.name as album_name, al.rating, row_number() over (partition by a.id order by al.ratign desc) as rn
    from artist a
    join song s on a.id = s.artist_id
    join album al on s.album_id = al.id
)
select artist_name, album_name, 'best rated album is ' || album_name as best_rated_album_text
from BestRatedAlbums
where rn = 1;

-- 4. Create a temp table with playlist that has songs which are in albums which are good rated (4.5+ rating)
create temp table good_rated_playlists as
...
-- 5. Create a function that will provide artist name, concatenated genre names he has songs in.
create or replace function get_artist_genres()
returns table (
    artist_name varchar,
    genre_names text
) as $$
...
-- 6. Create a function that will provide: Number of songs per album, number of songs per playlist, number of songs per genre
create or replace function get_song_counts()
returns table (
    album_id int,
    album_name varchar,
    song_count_per_album int,
    playlist_id int,
    playlist_title varchar,
    song_count_per_playlist int,
    genre_id int,
    genre_name varchar,
    song_count_per_genre int
) as $$
begin
    return query 
    select 
        ...
        count(s.id) as song_count_per_album,
        null::int as playlist_id,
        null::varchar as playlist_title,
        null::int as song_count_per_playlist,
        null::int as genre_id,
        null::varchar as genre_name,
        null::int as song_count_per_genre
    from 
        album al
    left join 
    ...
    union all
    select 
        null::int as album_id,
        null::varchar as album_name,
        null::int as song_count_per_album,
        p.id as playlist_id,
        p.title as playlist_title,
        count(ps.song_id) as song_count_per_playlist,
        null::int as genre_id,
        null::varchar as genre_name,
        null::int as song_count_per_genre
    from 
    ...
    select 
        ...
        count(sg.song_id) as song_count_per_genre
    from 
        genre g
    left join 
...
-- 1. Create a function that returns all information from the artist details table for a given artist id, using a row variable.
create or replace function get_artist_details(artist_id int)
returns artist_details as 
$$
declare artist_record artist_details%rowtype;
begin
	select *
	into artist_record
	from artist_details
	where artist_id = get_artist_details.artist_id;
    return artist_record;
end;
$$ language plpgsql;

-- 2. Create a function doing the same thing as above but using a record.
create or replace function get_artist_details_record(artist_id int)
returns record as $$
declare 
...
-- 3. Create a function that will return all rock songs, determined by the artist id, with all of the artist details.

-- 4. Create a function that returns the quantity of songs for a given album ID, only if the quantity is bigger than 5. If it is not, raise a notice. Use the if/else statement.
create or replace function get_song_count_if_greater_than_5(album_id int)
returns int as $$
declare
    song_count int;
begin
    ...
    into song_count
    from song
    where album_id = get_song_count_if_greater_than_5.album_id;
    if song_count > 5 then
        return song_count;
    else
        -- Raise a notice if the song count is not greater than 5
        raise notice 'The album with ID % has only % songs, which is not greater than 5.', album_id, song_count;
        return null; -- You can return a specific value or null if you prefer
    end if;
end;
$$ language plpgsql;

-- 5. Create a function that returns all artists that sang a song from a playlist with a certain id.
create or replace function get_artists_from_playlist(playlist_id int)
returns table (
    artist_id int,
    artist_name varchar
) as $$
begin
    ...
    join 
        song s on a.id = s.artist_id
    join 
        playlists_songs ps on s.id = ps.song_id
    join 
        playlist p on ps.playlist_id = p.id
    where 
        p.id = get_artists_from_playlist.playlist_id;
end;
$$ language plpgsql;
